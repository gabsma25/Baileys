...
Clock AnimationManager::m_clock;
...
map<string, float> AnimationManager::m_frequencies;

map<string, Clock> AnimationManager::m_timesClocks;
map<string, int> AnimationManager::m_timesUpdated;
map<string, float> AnimationManager::m_lastUpdateTime;
...
void AnimationManager::update(string animation, Sprite &sprite) {
    ...
    if (m_frequencies[animation] == 0) {
        m_frequencies[animation] = 5.f/60.f;
    }

    if (m_lastUpdateTime[animation] == 0) {
        m_lastUpdateTime[animation] = m_timesClocks[animation].restart().asSeconds();
        return;
    }

    if (m_lastUpdateTime[animation] < m_frequencies[animation]) {
        m_lastUpdateTime[animation] += m_timesClocks[animation].restart().asSeconds();
        return;
    }
    m_lastUpdateTime[animation] = 0;
    ...
    if (m_sheetSizes[animation] != Vector2i(0, 0)) {
        ...
        Vector2i size(m_spriteSizes[animation].x, m_spriteSizes[animation].y);
        ...
        // Update the Times Updated counter
